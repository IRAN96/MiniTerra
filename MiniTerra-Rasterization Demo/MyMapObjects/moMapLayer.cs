...
namespace MyMapObjects
{
    public class moMapLayer
    {
        ...
        private moGeometryTypeConstant _ShapeType = moGeometryTypeConstant.Point;//要素类型
        private string _Name = "Untitled";//图层名称
        private bool _Visible = true;//是否可见
        private bool _Selectable = true;//是否可以执行选择操作
        private string _Description="";//图层的描述
        ...
        {
            throw new NotImplementedException();//自行实现
        }
        /// <summary>
        /// 清除选择
        /// </summary>
        ...
        {
            //（1）加入AttributesFields对象的事件
            _AttributeFields.FieldAppended += _AttributeFileds_FieldAppended;
            _AttributeFields.FieldRemoved += _AttributeFileds_FieldRemoved;
            //（2）初始化图层的渲染
            InitializeRenderer();
        }
